...
export const selectVariants = tv({
  slots: {
    triggerRoot: [
      // base
      'group/trigger min-w-0 shrink-0 bg-bg-white shadow-regular-xs outline-none ring-1 ring-inset ring-stroke-soft-200 cursor-pointer',
      'text-paragraph-sm text-text-strong',
      ...
      'focus:text-text-strong data-[placeholder]:focus:text-text-strong',
      // disabled
    ...
    triggerArrow: [
      ...
      // focus
      'group-focus/trigger:text-text-strong group-data-[placeholder]/trigger:group-focus/trigger:text-text-strong',
      // disabled
    ...
    triggerIcon: [
      ...
      // disabled
      'group-disabled/trigger:text-text-disabled-300 group-data-[placeholder]/trigger:group-disabled/trigger:text-text-disabled-300',
      'group-disabled/trigger:[&:not(.remixicon)]:opacity-50',
    ],
    selectItemIcon: [
      'size-5 shrink-0 bg-[length:1.25rem] text-text-sub-600',
      '[[data-disabled]_&:not(.remixicon)]:opacity-50 [[data-disabled]_&]:text-text-disabled-300',
    ],
  },
  variants: {
    size: {
    ...
    variant: {
      ...
      inline: {
        ...
        triggerArrow: [
          'ml-0.5',
          'group-hover/trigger:text-text-strong',
          'group-data-[state=open]/trigger:text-text-strong',
        ],
  ...
  compoundVariants: [
    ...
    },
    {
      ...
      variant: 'default',
      class: {
        triggerRoot: 'h-8 min-h-8 gap-1.5 rounded-lg pl-2 pr-1.5 text-label-xs',
      },
    },
    {
      size: 'medium',
    ...
    },
    {
      ...
      variant: 'compact',
      class: {
        triggerRoot: 'h-8 gap-0.5 rounded-lg pl-2.5 pr-1.5 text-label-xs',
...
function TriggerIcon<T extends React.ElementType = 'div'>({
  ...
  return <Component className={triggerIcon({ class: className as string })} {...rest} />;
}
...
>(
  (
    { className, position = 'popper', children, sideOffset = 6, collisionPadding = 8, ...rest },
    forwardedRef,
  ) => (
    <SelectPrimitives.Portal>
      <SelectPrimitives.Content
        ref={forwardedRef}
        className={cnExt(
          'relative z-50 overflow-hidden rounded-2xl bg-bg-white shadow-regular-md ring-1 ring-inset ring-stroke-soft-200',
        ...
        {...rest}
      >
        <SelectPrimitives.Viewport className="max-h-[220px] w-full overflow-y-auto p-1.5">
          {children}
...
>(({ className, children, ...rest }, forwardedRef) => {
  ...
  return (
    <SelectPrimitives.Item
      ref={forwardedRef}
      className={cnExt(
        'group relative cursor-pointer select-none rounded-lg p-2 pr-9 text-paragraph-sm text-text-strong outline-none',
        ...
        {
          'gap-1.5 pr-8 text-label-xs': size === 'xsmall',
        },
        className,
      )}
      {...rest}
    >
      <SelectPrimitives.ItemText asChild>
        <span
          className={cn('flex flex-1 items-center gap-2', 'group-disabled:text-text-disabled-300', {
            'gap-1.5': size === 'xsmall',
          })}
        >
      ...
      <SelectPrimitives.ItemIndicator asChild>
        <RiCheckLine className="absolute right-2 top-1/2 size-4 shrink-0 -translate-y-1/2 text-text-sub-600" />
...
function SelectItemIcon<T extends React.ElementType>({
  ...
  return <Component className={selectItemIcon({ class: className as string })} {...rest} />;
}
